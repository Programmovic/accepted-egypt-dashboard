  ...
  const handleUpdateBatchesStatus = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        // You can also choose to close the edit modal at this point
        fetchBatchData(); // Fetch updated batch data
        toast.success("Batches Status updated successfully!");
      } else {
        toast.error("Failed to update the batches status");
      }
    } catch (error) {
      console.error("Error updating batch:", error);
      toast.error("Failed to update the batch");
              ...
              <Col xs={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    type="date"
                    required
                    value={new Date(editingBatch?.shouldStartAt)
                      .toLocaleDateString()
                      .replace(/\//g, "-")}
                  />
                </Form.Group>
        ...
        <Card.Body>
          <Form className="mb-3">
            ...
            <Row>
              ...
              <Col xs={4}>
                <Button variant="primary" onClick={handleUpdateBatchesStatus}>
              ...
              <Table striped bordered hover>
                <thead>
                  <tr>
                    ...
                    <th onClick={() => handleSort("trainees")}>Trainees</th>
                ...
                <tbody>
                  {sortedBatches.filter((batch) => batch.status === "Ended").map((batch, index) => (
                    ...
                    >
                      ...
                      <td>{batch.studentCount} Trainees</td>
                      <td>
