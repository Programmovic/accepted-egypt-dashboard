...
import CandidateStatusForRecruiter from "../../../models/candidateStatusForRecruiter";
...
const createCandidateStatus = async (req, res) => {
  try {
    ...
    const candidateStatus = new CandidateStatusForRecruiter({ status, description });
...
const readCandidateStatuses = async (req, res) => {
  try {
    const candidateStatuses = await CandidateStatusForRecruiter.find();
...
const updateCandidateStatus = async (req, res) => {
  ...
  try {
    const updatedCandidateStatus = await CandidateStatusForRecruiter.findByIdAndUpdate(id, updateData, { new: true });
...
const deleteCandidateStatus = async (req, res) => {
  ...
  try {
    const candidateStatus = await CandidateStatusForRecruiter.findByIdAndDelete(id);
    if (!candidateStatus) {
      return res.status(404).json({ error: "Candidate status not found" });
    }
